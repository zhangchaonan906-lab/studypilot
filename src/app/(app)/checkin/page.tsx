...
import { getCheckinStats, getMonthlyCheckins } from "@/lib/study/checkin";
...
export default async function CheckinPage() {
  ...
  let stats = { monthTotal: 0, streak: 0, todayCheckedIn: false };
  let dbError: string | null = null;

  try {
    const [dates, statsResult] = await Promise.all([
    ...
    if (message.includes("Could not find the table") || message.includes("schema cache")) {
      ...
      dbError = message || "加载打卡记录失败，请稍后重试。";
    }
  }
  ...
  return (
    <>
      <PageHeader
        eyebrow="猫爪打卡"
        title="记录你的每日坚持"
        description="每天完成学习后点一下打卡，让猫爪记录你的坚持。"
      />
